  ...
  for i, image_scale in enumerate(eval_scales):
    ...
    for output in sorted(outputs_to_scales_to_logits):
      ...
      if add_flipped_images:
        ...
        logits_reversed = tf.image.resize(
  ...
  for image_scale in image_pyramid:
    if image_scale != 1.0:
      ...
      scaled_images = tf.image.resize(
          images, scaled_crop_size, method=tf.image.ResizeMethod.BILINEAR)
    ...
    for output in sorted(outputs_to_logits):
      outputs_to_logits[output] = tf.image.resize(
...
def _get_logits(images,
  ...
  if model_options.decoder_output_stride is not None:
    if model_options.crop_size is None:
      height = tf.shape(input=images)[1]
      width = tf.shape(input=images)[2]
    else:
    ...
    with slim.arg_scope([slim.batch_norm], **batch_norm_params):
      with tf.compat.v1.variable_scope(DECODER_SCOPE, DECODER_SCOPE, [features]):
        ...
        if feature_list is None:
          tf.compat.v1.logging.info('Not found any decoder end points.')
          return features
        else:
          ...
          for i, name in enumerate(feature_list):
            ...
            for j, feature in enumerate(decoder_features_list):
              decoder_features_list[j] = tf.image.resize(
                  feature, [decoder_height, decoder_width], method=tf.image.ResizeMethod.BILINEAR)
