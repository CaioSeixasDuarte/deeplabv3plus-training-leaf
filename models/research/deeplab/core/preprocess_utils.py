...
def randomly_scale_image_and_label(image, label=None, scale=1.0):
  ...
  if label is not None:
    label = tf.squeeze(tf.image.resize(
...
def resize_to_range(image,
  ...
  with tf.compat.v1.name_scope(scope, 'resize_to_range', [image]):
    new_tensor_list = []
    min_size = tf.cast(min_size, dtype=tf.float32)
    if max_size is not None:
      max_size = tf.cast(max_size, dtype=tf.float32)
    ...
    orig_height = tf.cast(orig_height, dtype=tf.float32)
    orig_width = tf.cast(orig_width, dtype=tf.float32)
    ...
    large_height = tf.cast(tf.math.ceil(orig_height * large_scale_factor), dtype=tf.int32)
    large_width = tf.cast(tf.math.ceil(orig_width * large_scale_factor), dtype=tf.int32)
    ...
    if max_size is not None:
      ...
      small_height = tf.cast(tf.math.ceil(orig_height * small_scale_factor), dtype=tf.int32)
      small_width = tf.cast(tf.math.ceil(orig_width * small_scale_factor), dtype=tf.int32)
      small_size = tf.stack([small_height, small_width])
      new_size = tf.cond(
          pred=tf.cast(tf.reduce_max(input_tensor=large_size), dtype=tf.float32) > max_size,
    ...
    if label is not None:
      if label_layout_is_chw:
        ...
        resized_label = tf.image.resize(
            resized_label, new_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
