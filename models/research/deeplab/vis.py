...
def main(unused_argv):
  ...
  with g.as_default():
    ...
    sv = tf.compat.v1.train.Supervisor(graph=g,
    ...
    while (FLAGS.max_number_of_iterations <= 0 or
      ...
      tf.compat.v1.logging.info(
          'Starting visualization at ' + time.strftime('%Y-%m-%d-%H:%M:%S',
                                                       time.gmtime()))
      tf.compat.v1.logging.info('Visualizing with model %s', last_checkpoint)
      ...
      with sv.managed_session(FLAGS.master,
        ...
        for batch in range(num_batches):
          tf.compat.v1.logging.info('Visualizing batch %d / %d', batch + 1, num_batches)
